...
export const SuppliersTab: React.FC = () => {
    ...
    const renderAccordionView = () => (
        <Accordion type="multiple" className="space-y-4" defaultValue={suppliers.map(s => s.id!)}>
            {suppliers.map(s => (
                <AccordionItem key={s.id} value={s.id!} className="bg-card rounded-xl border-b-0 shadow-sm">
                    ...
                    <AccordionContent className="px-6 pb-4">
                       <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                            <div>
                                {s.contactPerson && <p className="text-muted-foreground">Contato: {s.contactPerson}</p>}
                                {s.phone && <p className="text-muted-foreground">Telefone: {s.phone}</p>}
                            </div>
                            <div className="flex items-center justify-end space-x-1 shrink-0">
                                <Button onClick={() => handleOpenModal('purchase', s)} variant="outline" size="sm" className="text-accent border-accent hover:bg-accent/10 hover:text-accent/80"><ShoppingCart size={16} className="mr-2"/> Registrar Compra</Button>
                                <Button onClick={() => handleOpenModal('history', s)} variant="outline" size="sm"><FileText size={16} className="mr-2"/> Histórico</Button>
                                <Button onClick={() => handleOpenModal('form', s)} variant="outline" size="sm" className="text-primary border-primary hover:bg-primary/10 hover:text-primary/80"><Edit size={16} className="mr-2"/> Editar</Button>
                                <Button onClick={() => handleDeleteClick(s)} variant="outline" size="sm" className="text-destructive border-destructive hover:bg-destructive/10 hover:text-destructive/80"><Trash2 size={16} className="mr-2"/> Excluir</Button>
                            </div>
                       </div>
                    </AccordionContent>
                </AccordionItem>
        ...
        <>
            <div className="p-1 md:p-4">
                <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                    <h2 className="text-3xl font-bold text-foreground flex items-center"><Truck className="mr-3" /> Gerenciar Fornecedores</h2>
                    <Button onClick={() => handleOpenModal('form')} className="bg-primary text-primary-foreground font-semibold hover:bg-primary/80 w-full md:w-auto">
