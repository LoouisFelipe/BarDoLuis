...
export function FinancialsTab() {
    ...
    const form = useForm({ defaultValues: { description: '', amount: '', category: '', expenseDate: new Date().toISOString().split('T')[0], replicate: false, monthsToReplicate: '11' } });
    ...
    const stats = useMemo(() => {
        ...
        return { income, expense, balance: income - expense, receivable: accountsReceivable };
    ...
    const handleAddExpense = async (data: any) => {
        ...
        try {
            ...
            await addExpense(data.description, amountVal, data.category || 'Geral', data.expenseDate, replicateCount);
        ...
        <TooltipProvider>
            <div className="p-1 md:p-4 space-y-6 pb-24">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <Card className="bg-red-500/5 border-red-500/20 p-4 rounded-2xl"><div className="flex items-center gap-2 mb-1"><TrendingDown size={14} className="text-red-500" /><span className="text-[9px] font-black uppercase text-muted-foreground">Saídas</span></div><div className="text-xl font-black text-red-500">R$ {stats.expense.toFixed(2)}</div></Card>
                    <Card className="bg-primary/5 border-primary/20 p-4 rounded-2xl"><div className="flex items-center gap-2 mb-1"><Scale size={14} className="text-primary" /><span className="text-[9px] font-black uppercase text-muted-foreground">Líquido</span></div><div className="text-xl font-black text-primary">R$ {stats.balance.toFixed(2)}</div></Card>
                    <Card className="bg-yellow-500/5 border-yellow-500/20 p-4 rounded-2xl"><div className="flex items-center gap-2 mb-1"><Users size={14} className="text-yellow-500" /><span className="text-[9px] font-black uppercase text-muted-foreground">A Receber</span></div><div className="text-xl font-black text-yellow-400">R$ {stats.receivable.toFixed(2)}</div></Card>
                </div>
